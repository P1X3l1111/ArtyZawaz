...
export default function ProduseNoi() {
  ...
  return (
    <section style={{ background: "#f9f8f6", padding: "72px 0", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 48, flexWrap: "nowrap" }} className="ps-row">
        ...
        <div style={{ flexShrink: 0, paddingLeft: "clamp(24px, 5vw, 80px)" }} className="ps-panel">
          <h2 style={{ fontSize: "clamp(28px, 3.5vw, 56px)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 6px", color: "#111", whiteSpace: "nowrap" }}>Produse Noi</h2>
          <p style={{ fontSize: "clamp(13px, 1.1vw, 17px)", color: "#666", margin: "0 0 20px", lineHeight: 1.4, whiteSpace: "nowrap" }}>Cele mai recente adăugări în colecția noastră</p>
        ...
        <div style={{ flex: 1, minWidth: 0 }} className="ps-cards-wrap">
          <div
            ref={scrollRef}
            className="ps-cards"
            style={{ display: "flex", gap: "clamp(14px, 1.5vw, 24px)", overflowX: "hidden", scrollbarWidth: "none", msOverflowStyle: "none", cursor: dragRef.current.dragging ? "grabbing" : "grab", userSelect: "none" }}
      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: 36 }}>
        <Link href="/produse-noi" style={{ padding: "12px 36px", border: "2px solid #111", borderRadius: 999, fontSize: 14, fontWeight: 700, color: "#111", textDecoration: "none", transition: "all 0.2s", background: "transparent" }}
          onMouseEnter={e => { e.currentTarget.style.background = "#111"; e.currentTarget.style.color = "#fff"; }}
          onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#111"; }}
        >Vezi toate</Link>
      </div>
