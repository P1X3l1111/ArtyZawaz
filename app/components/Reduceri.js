...
export default function Reduceri() {
  ...
  useEffect(() => {
    fetch("/api/produse").then(r => r.json()).then(data => {
      const filtered = data.filter(p => p.tags?.includes("reduceri"));
  ...
  return (
    <section style={{ background: "#fff", padding: isMobile ? "48px 0" : "72px 0", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 48, flexWrap: "nowrap" }} className="ps-row">
        ...
        {/* Right: text + controls */}
        <div style={{ flexShrink: 0, paddingRight: "clamp(24px, 5vw, 80px)", textAlign: "right" }} className="rs-panel">
